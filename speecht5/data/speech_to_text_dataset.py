...
class SpeechToTextDataset(FairseqDataset):
    ...
    def get_label(self, index, label_idx):
        ...
        constraints_ = label.split('\t')
        label = constraints_.pop(0)

        constraint_vec = torch.tensor([0])  # Raghu changed - Added
        if self.include_constraints is not None:
            constraint_vec = [len(constraints_)]
            for constraint in constraints_:
                constraint_vec += self.label_processors[label_idx](self.tokenizer.encode(constraint))[1:]
                constraint_vec += [0]
            constraint_vec = torch.tensor(constraint_vec)
        #print("\n".join(constraints_), constraint_vec)
        ...
        return constraint_vec, label #Raghu

    def get_labels(self, index): #Raghu changed
        constraints = []
        labels = []
        for i in range(self.num_labels):
            constraint_vec, label = self.get_label(index, i) 
    ...
    def __getitem__(self, index):
        ...
        constraint , labels = self.get_labels(index) #Raghu Changed
        if self.include_constraints is not None: # Raghu changed
            return {"id": index, "source": wav, "label_list": labels, "constraints":constraint[0]}
        else:
    ...
    def collater(self, samples):
        ...
        #constraints = [s["constraints"] for s in samples]
